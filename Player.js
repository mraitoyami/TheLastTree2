...
export class Player {
    constructor() {
        this.stats = {
            ...
            efficiency: 1.0, // Multiplier for oxygen loss
        ...
        if (this.stats.oxygen < CONFIG.STATS.CRITICAL_OXYGEN) {
            ...
            this.mesh.traverse(child => {
                if (child.isMesh && child.material.emissive) {
                    child.material.emissive.setRGB(t * 0.5, 0, 0);
                }
            });
        } else {
            this.isBurning = false;
            // Restore emissive if not burning
            this.mesh.traverse(child => {
                if (child.isMesh && child.material.emissive && child !== this.weaponMesh.children[1]) {
    ...
    updateStats(deltaOxygen, deltaMoney = 0, deltaWood = 0, deltaGarbage = 0, deltaDurability = 0) {
        if (this.stats.isDead) return;
        ...
        // Update hunt mesh visibility if upgraded
